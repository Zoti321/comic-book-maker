...
| 操作结果（非阻塞） | `ScaffoldMessenger.showSnackBar` | 成功提示、可忽略的失败 |
| 需确认或展示结果摘要 | `showDialog` + `AlertDialog` | 导出成功路径、导入结果、创建失败详情 |
...
- **操作按钮**：宽屏导出 `FilledButton`、追加导入 `OutlinedButton`（**40px** 高、M3 桌面标准、宽度随文案）；窄屏分别为 `IconButton.filled` / `IconButton` + tooltip；项目属性圆形 `IconButton`（`Icons.settings_outlined`，compact）。图标均用 Material Icons（header + Tab 行）。
...
- **年龄分级**：`MetadataAgeRatingField`（`DropdownButtonFormField`）；选项仅 Core `ageRatingPresets` 四项，不可自由输入；未选占位「未设置」，有值时 suffix ✕ 清空为 `NULL`。Core `age_rating` 模块负责导入/加载别名归一化；`mergeMetadataFromForm` 保存时校验非空值必须在预设内。

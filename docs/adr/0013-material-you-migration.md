...
- 元数据 Tab：`dropdown_button2`（年龄分级）、`textfield_tags`（逗号标签）保留；仅通过 `Theme` / `InputDecorationTheme` 贴近 M3；原生替换见 follow-up issue #9。
...
- 负面：`design_system` 测试与目录仍存直至根级迁移完成；`dropdown_button2` / `textfield_tags` 仍非纯 Material。

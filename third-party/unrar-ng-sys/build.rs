fn main() {
    ...
    // build.rs runs on the host, so `cfg!(windows)` / `#[cfg(windows)]` refer
    // to the build machine — not the compilation target. Use Cargo's target
    // cfg env vars so Windows-only UnRAR sources are excluded when
    // cross-compiling to Android/iOS/Linux from Windows.
    ...
    if target_is_windows {
        ...
        // Android uses bionic (pthread in libc); macOS/iOS don't use this branch.
        println!("cargo:rustc-link-lib=pthread");
    ...
    if target_is_windows {
        file_stems.push("motw"); // Mark of the Web support (Windows only)
    }
    ...
    // Android exposes utimensat but not lutimes unless _BSD_SOURCE is set.
    // Prefer the nanosecond path already used on modern Linux/macOS.
    if target_os == "android" {
        build.define("UNIX_TIME_NS", None);
    }
